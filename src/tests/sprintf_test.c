...
START_TEST(test_s21_sprintf_multiple_flags) {
  ...
  sprintf(expected_output, "Integer: %+d, Float: %010.4f, String: %-10s",
          intValue, floatValue, stringValue);
  s21_sprintf(buffer, "Integer: %+d, Float: %010.4f, String: %-10s", intValue,
...
START_TEST(test_s21_sprintf_complex) {
  ...
  sprintf(expected_output, "Int: %+010d, Float: %.3e, String: %-20s", intValue,
          floatValue, stringValue);
  s21_sprintf(buffer, "Int: %+010d, Float: %.3e, String: %-20s", intValue,
              floatValue, stringValue);
...
START_TEST(test_s21_sprintf_alignment) {
  ...
  sprintf(expected_output, "Int: %*d, Float: %*.2f, String: %*s", 8, intValue,
          10, floatValue, 15, stringValue);
  s21_sprintf(buffer, "Int: %*d, Float: %*.2f, String: %*s", 8, intValue, 10,
              floatValue, 15, stringValue);
...
START_TEST(test_s21_sprintf_random_float) {
  ...
  double floatValue =
      (rand() % 10000) / 100.0 -
      50.0;  // Случайное число с плавающей точкой от -50.0 до 49.99
  char expected_output[MAX_BUFFER_SIZE];
  char buffer[MAX_BUFFER_SIZE];
...
//     sprintf(expected_output, "Unsigned long overflow: %lu",
//     18446744073709551616); s21_sprintf(buffer, "Unsigned long overflow: %lu",
//     18446744073709551616); ck_assert_str_eq(buffer, expected_output);
// }
// END_TEST
...
// Тестовый случай для s21_sprintf с использованием спецификатора "ls" и нулевой
// широкой строки
