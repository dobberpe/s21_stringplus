...
START_TEST(test_apply_format_s_basic) {
  ...
  test_all_falgs_string(mod, "-^&%!^#&YH%T!O#H!#HG!*&(Y)(&$!@T$)",
                        "-^&%!^#&YH%T!O#H!#HG!*&(Y)(&$!@T$)", 's');
}
END_TEST
...
void test_all_falgs_double(print_modifiers mod, char *src, double src2,
  ...
  for (int i = 0; i < 32; i++) {
    ...
    // DEBUG
    // str = (char *)realloc(str, (s21_strlen(str) + s21_strlen(format) + 1) *
    // sizeof(char)); str = strcat(str, format); str2 = (char *)realloc(str2,
    // (s21_strlen(str2) + s21_strlen(format) + 1) * sizeof(char)); str2 =
    // strcat(str2, format); END DEBUG
...
char *format_maker(print_modifiers *mod, char ch, char len) {
  ...
  } else if (mod->fill_with_nulls && s21_strchr("dieEfgGouxX", ch) &&
             !(s21_strchr("idouxX", ch) && mod->precision != 1)) {
    res[i++] = '0';
  } else {
    mod->fill_with_nulls = false;
  }
  if (mod->width != 0) res[i++] = mod->width + '0';
  if (s21_strchr("diouxXeEfgGs", ch) &&
      ((s21_strchr("eEfgG", ch) && mod->precision != 6) ||
       (s21_strchr("diouxX", ch) && mod->precision != 1) ||
       (ch == 's' && mod->precision != -1))) {
    res[i++] = '.';
