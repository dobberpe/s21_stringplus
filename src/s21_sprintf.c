...
int process_format(const char *format, int *i, char *str, va_list *params,
                   print_modifiers *format_modifiers) {
  ...
  bool fail = false;
  char *res = process_specifier(format[*i], s21_strlen(str), params,
                                format_modifiers, &fail);
  if (res) {
...
char *process_specifier(char specifier, const int len, va_list *params,
                        print_modifiers *format_modifiers, bool *fail) {
  char *res = NULL;
  if (specifier == 'c') {
    res = get_c(params, format_modifiers->length, fail);
  } else if (specifier == 'd' || specifier == 'i') {
    format_modifiers->precision =
        format_modifiers->precision == -1 ? 1 : format_modifiers->precision;
    res = ditoa(format_modifiers->length == 'h'   ? (short)va_arg(*params, int)
                : format_modifiers->length == 'l' ? va_arg(*params, long)
                                                  : va_arg(*params, int));
  } else if (specifier == 'e' || specifier == 'E' || specifier == 'f' ||
             specifier == 'g' || specifier == 'G') {
    format_modifiers->precision =
        format_modifiers->precision == -1 ? 6 : format_modifiers->precision;
    res = get_f(params, format_modifiers->length);
  } else if (specifier == 'o' || specifier == 'u' || specifier == 'x' ||
             specifier == 'X') {
    format_modifiers->precision =
        format_modifiers->precision == -1 ? 1 : format_modifiers->precision;
    res = uoxtoa(format_modifiers->length == 'h'
                     ? (unsigned short)va_arg(*params, unsigned int)
                 : format_modifiers->length == 'l'
                     ? va_arg(*params, unsigned long)
                     : va_arg(*params, unsigned),
                 specifier == 'o'   ? 8
                 : specifier == 'u' ? 10
                                    : 16,
                 specifier == 'X');
  ...
  return res && specifier != '%' && specifier != 'n'
             ? apply_format(res, *format_modifiers, specifier)
             : res;
}
...
char *get_f(va_list *params, char length) {
  ...
  if (length == 'L') {
    ...
    if (isinf(f.full) || isnan(f.full))
      res = edge_case(isinf(f.full), f.bits & 0x8000000000000000);
    else
      res = ftoa(&f);
  }
...
char *ftoa(const f_representation *f) {
  ...
  if (f->full) {
    integer = e >= 0 ? calculate_int_part(integer, e, f->bits,
                                          e > 52 ? 1 : mask >> (e - 1))
                     : integer;
    mask = e > 0 ? mask >> e : mask;
    fraction =
        mask ? calculate_frac_part(fraction, e, f->bits, mask) : fraction;
  }
...
char *calculate_int_part(char *integer, const int e,
...
char *calculate_frac_part(char *fraction, int e, const unsigned long long bits,
                          unsigned long long mask) {
...
char *augment_frac(char *fraction, int e, char **power_of_5, int *prev_p) {
  ...
  *prev_p = e;
  ...
  fraction = fraclen < addlen
                 ? add_width(fraction, addlen - fraclen, '0', false)
                 : fraction;
  char *tmp = fraction;
...
char *join_frac_to_int(char *integer, char *fraction, bool negative) {
  ...
  char *tmp = res;
  res = (char *)realloc(
      res, (s21_strlen(res) + s21_strlen(fraction) + 1) * sizeof(char));
  if (!res)
    free(tmp);
  else
...
char *lftoa(const fl_representation *f) {
  ...
  if (f->full) {
    integer = e >= 0 ? calculate_int_part_long(integer, e, f->bits,
                                               e > 62 ? 1 : 0x8000 >> (e % 16))
                     : integer;
    unsigned short mask = 0x8000 >> (e % 16 + 1);
    fraction = e < 63 ? calculate_frac_part_long(fraction, e, f->bits,
                                                 mask ? mask : 0x8000)
                      : fraction;
  }
...
char *calculate_int_part_long(char *integer, const int e,
                              const unsigned short *bits, unsigned short mask) {
  int p = e > 64 ? e - 63 : 0;
  char *power_of_2 = ditoa(1);
  int prev_p = 0;
  int i = 2 - e / 16;
...
char *calculate_frac_part_long(char *fraction, int e,
...
char *etoa(char *f_str, print_modifiers format_modifiers, char specifier) {
  ...
  while (f_str[++integer_part] && f_str[integer_part] < '1')
    ;
  int exp =
      point > integer_part ? point - integer_part - 1 : point - integer_part;
...
char *string_format(char *str, int *exp, print_modifiers format_modifiers,
                    char specifier) {
  ...
  if (str[1] == '0' || (k == '9' && *str == '1')) {
    ...
    } else if (!format_modifiers.oct_hex_notation ||
               !format_modifiers.precision) {
      str[1] = '\0';
    } else {
      str[1] = '.';
...
void g_precision_calc(char *str, print_modifiers *format_modifiers) {
  ...
  if (*tmp == '\0') {
    ...
    if (dot_find) {
      ...
      if (format_modifiers->precision == 0 && offset != 1)
        format_modifiers->precision += 1;
      format_modifiers->precision +=
          offset - format_modifiers->oct_hex_notation;
    } else {
...
char *double_round(char *str, print_modifiers format_modifiers,
                   char specifier) {
    ...
    *tmp = '\0';
    ...
    if (val > '4' && val <= '9') {
      if (format_modifiers.precision == 0) {
        ...
        char *str2 =
            (char *)malloc((3 + format_modifiers.precision) * sizeof(char));
      ...
      if (s21_strchr("gG", specifier) && s21_strlen(tmp) > s21_strlen(str))
        tmp[s21_strlen(tmp) - 1] = '\0';
      free(str);
      str = tmp;
    }
  }
...
char *set_precision(char *str, print_modifiers format_modifiers,
                    char specifier) {
  int str_len = (int)s21_strlen(str);
  if (specifier == 's' && format_modifiers.precision >= 0 &&
      str_len > format_modifiers.precision) {
    str[format_modifiers.precision] = '\0';
  } else if (s21_strchr("diouxX", specifier) &&
             str_len < format_modifiers.precision) {
    str = add_width(str, format_modifiers.precision - str_len, '0', true);
  } else if (s21_strchr("diouxX", specifier) && *str == '0' &&
             format_modifiers.precision == 0) {
    *str = '\0';
    ...
    if ((exp < -4 || exp >= format_modifiers.precision) &&
        !(!exp && !format_modifiers.precision &&
          !(str[0] == '9' && str[2] > '4'))) {
      str = etoa(str, format_modifiers, specifier);
    } else {
      str = double_round(str, format_modifiers, specifier);
...
char *set_hex_notation(char *str, print_modifiers format_modifiers,
                       char specifier, char is_zero) {
  size_t str_len = s21_strlen(str);
  if ((format_modifiers.oct_hex_notation &&
       s21_strchr("oxXeEfgG", specifier)) ||
      specifier == 'p') {
    if (specifier == 'o' && *str != 0 && *str != '0') {
      ...
      str = add_width(str, 1, 'X', true);
      str = add_width(str, 1, '0', true);
    } else if (s21_strchr("fgG", specifier) && !s21_strpbrk(str, "eE") &&
               !s21_strpbrk(str, ",.")) {
      str = add_width(str, 1, '.', false);
    } else if (s21_strchr("gGeE", specifier) && s21_strpbrk(str, "eE") &&
               !s21_strpbrk(str, ",.")) {
...
char *set_width(char *str, print_modifiers format_modifiers, char specifier) {
  ...
  if (format_modifiers.width > str_len &&
      (!format_modifiers.fill_with_nulls || s21_strchr("(in", *str) ||
       (*str != '\0' && str[1] != '\0' && s21_strchr("(in", str[1])))) {
    str = add_width(str, format_modifiers.width - str_len, ' ',
                    !format_modifiers.left_alignment);
  } else if (format_modifiers.width > str_len &&
             format_modifiers.fill_with_nulls) {
...
char *apply_format(char *str, print_modifiers format_modifiers,
                   char specifier) {
  char is_zero = *str;
  size_t str_len = s21_strlen(str);
  ...
  if (negative) {
    ...
    str = add_width(str, 1, '+', true);
  } else if (format_modifiers.space_instead_of_sign &&
             !format_modifiers.positive_sign &&
             s21_strchr("dieEfgG", specifier)) {
    str = add_width(str, 1, ' ', true);
  }
  // ширина
